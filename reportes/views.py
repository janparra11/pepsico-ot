...
def export_pdf(request):
    ...
    if logo_path:
        try:
            ...
            c.drawImage(img, left, top - logo_h, width=logo_w, height=logo_h, preserveAspectRatio=True, mask='auto')
        except Exception:
            logo_w, logo_h = (0, 0)
    ...
    if filtros.get("rango"):
        ...
        c.drawString(left, y, f"Rango rápido: {etiqueta}"); y -= 0.5*cm
    c.drawString(left, y, f"Desde: {filtros['fini'] or '(todos)'}   Hasta: {filtros['ffin'] or '(todos)'}"); y -= 0.5*cm
    c.drawString(left, y, f"Estado: {filtros['estado'] or '(todos)'}   Taller: {filtros['taller'] or '(todos)'}   Mecánico: {filtros['mecanico'] or '(todos)'}"); y -= 0.8*cm

    # KPIs + SLA
    c.setFont("Helvetica-Bold", 10); c.drawString(2*cm, y, "KPIs:"); c.setFont("Helvetica", 10); y -= 0.5*cm
    c.drawString(2*cm, y, f"OTs abiertas: {stats['kpi_abiertas']}   OTs cerradas: {stats['kpi_cerradas']}   Tiempo prom: {stats['kpi_tiempo_prom']} h"); y -= 0.5*cm
    ahora = timezone.now()
    limite = ahora - timedelta(hours=getattr(cfg, "sla_horas", 0) or 0)
    vencidas = qs_ot.filter(activa=True, fecha_ingreso__lte=limite).count()
    c.drawString(2*cm, y, f"SLA: {getattr(cfg, 'sla_horas', 0) or 0} h   OTs vencidas: {vencidas}"); y -= 0.8*cm

    # Top Repuestos (con “Últ. mov.”)
    c.setFont("Helvetica-Bold", 10); c.drawString(2*cm, y, "Top 10 Repuestos (por consumo en filtros)"); y -= 0.6*cm
    c.setFont("Helvetica-Bold", 9)
    c.drawString(2*cm, y, "Código"); c.drawString(7*cm, y, "Descripción"); c.drawRightString(15*cm, y, "Cantidad"); c.drawRightString(19*cm, y, "Últ. mov.")
    y -= 0.45*cm; c.setFont("Helvetica", 9)

    ult_mov_map = {
        r["repuesto__codigo"]: r["ultimo"]
        for r in MovimientoStock.objects.filter(tipo=MovimientoStock.SALIDA)
                   .values("repuesto__codigo").annotate(ultimo=Max("creado_en"))
    }
    for r in list(stats["top_repuestos"][:10]):
        ...
        c.drawString(2*cm, y, cod[:20]); c.drawString(7*cm, y, desc); c.drawRightString(15*cm, y, f"{total}"); c.drawRightString(19*cm, y, ult_txt)
        y -= 0.42*cm
    ...
    c.setFont("Helvetica-Bold", 10); c.drawString(2*cm, y, "Top 10 Vehículos (por OTs en filtros)"); y -= 0.6*cm
    c.setFont("Helvetica-Bold", 9)
    c.drawString(2*cm, y, "Patente"); c.drawRightString(8*cm, y, "OTs"); c.drawString(9*cm, y, "Taller más frecuente"); c.drawRightString(16*cm, y, "Últ. servicio"); c.drawRightString(19*cm, y, "Hace")
    y -= 0.45*cm; c.setFont("Helvetica", 9)
    ...
    for v in veh_counts:
        ...
        if u:
            ...
            ult_txt = "—"
            hace_txt = "—"
        c.drawString(2*cm, y, p[:15]); c.drawRightString(8*cm, y, str(tot)); c.drawString(9*cm, y, (tall or "—")[:24]); c.drawRightString(16*cm, y, ult_txt); c.drawRightString(19*cm, y, hace_txt)
        y -= 0.42*cm
        if y < 2*cm: c.showPage(); y = h - 2*cm; c.setFont("Helvetica", 9)

    # Resumen por taller (alineado fijo)
    c.showPage(); y = h - 2*cm
    X_TALLER, X_TOTAL, X_ABIERT, X_CERRAD, X_PROM = 2*cm, 11*cm, 13*cm, 15*cm, 19*cm
    c.setFont("Helvetica-Bold", 12); c.drawString(X_TALLER, y, "Resumen por taller"); y -= 0.8*cm
    ...
    c.setFont("Helvetica-Bold", 9)
    c.drawString(X_TALLER, y, "Taller"); c.drawRightString(X_TOTAL, y, "Totales"); c.drawRightString(X_ABIERT, y, "Abiertas"); c.drawRightString(X_CERRAD, y, "Cerradas"); c.drawRightString(X_PROM, y, "T. prom (h)")
    y -= 0.5*cm; c.setFont("Helvetica", 9)

    def ellipsize(txt, max_len=38):
        if not txt: return "—"
        t = str(txt); return t if len(t) <= max_len else (t[:max_len-1] + "…")

    for row in agg_base:
        nombre = ellipsize(row["taller__nombre"] or "—")
        t = row["total_ots"] or 0; a = row["abiertas"] or 0; crr = row["cerradas"] or 0
        prom = dur_prom_map.get(row["taller__nombre"] or "—", 0.0)
        c.drawString(X_TALLER, y, nombre); c.drawRightString(X_TOTAL, y, f"{t:d}"); c.drawRightString(X_ABIERT, y, f"{a:d}"); c.drawRightString(X_CERRAD, y, f"{crr:d}"); c.drawRightString(X_PROM, y, f"{prom:.2f}")
        y -= 0.45*cm
        if y < 2*cm:
            c.showPage(); y = h - 2*cm
            c.setFont("Helvetica-Bold", 9)
